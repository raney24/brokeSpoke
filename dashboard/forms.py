...
class RawTransactionForm(forms.Form):
    ...
    paymentType         = forms.ChoiceField(label = "transaction Type", choices = PAYMENT_CHOICES )
...
class RawTimelogsForm(forms.Form):
    ...
    startTime           = forms.DateTimeField(initial=datetime.date.today)
...
class NewSignIn(forms.Form):
    ...
    person              = forms.CharField(label = "Person" )
    activity            = forms.ChoiceField(label = "Activity", choices = SIGN_IN_CHOICES )
    startTime           = forms.DateTimeField(label = "Start Time", initial=datetime.date.today)
